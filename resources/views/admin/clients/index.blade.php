...
<div class="container mx-auto">
    <div class="bg-gradient-to-r from-gray-900 via-black to-gray-900 relative rounded-lg mb-6 p-6">
        ...
        <div class="flex items-center justify-between relative">
            <h1 class="text-2xl font-bold text-white">Gestion des Clients</h1>
            <a href="{{ route('clients.create') }}" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-md flex items-center gap-2 transition-all">
    ...
    <div class="bg-red-900/30 border border-red-500/30 text-red-400 px-4 py-3 rounded-lg mb-6 flex items-center">
        ...
        <button class="ml-auto hover:text-red-200" onclick="this.parentElement.remove()">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
        </button>
    ...
    <div class="bg-gray-800 rounded-lg p-4 mb-6 border border-purple-500/10">
        <form action="{{ route('clients.index') }}" method="GET" class="flex flex-wrap gap-4">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('clients.index') }}" class="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-md transition-all">
                    Réinitialiser
                </a>
            </div>
        </form>
            ...
            <table class="min-w-full divide-y divide-gray-700">
                <thead>
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Commandes</th>
                ...
                <tbody class="divide-y divide-gray-700">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="w-10 h-10 flex-shrink-0 mr-3 bg-purple-600/20 rounded-full flex items-center justify-center text-purple-400 text-sm font-medium">
                                    {{ strtoupper(substr($client->nom ?? 'U', 0, 2)) }}
                                </div>
                                <div class="text-sm font-medium text-white">{{ $client->nom ?? 'Non renseigné' }}</div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{{ $client->email }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{{ $client->telephone ?? 'Non renseigné' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{{ $client->orders->count() }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <a href="{{ route('clients.show', $client) }}" class="text-blue-400 hover:text-blue-300" title="Voir">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <a href="{{ route('clients.edit', $client) }}" class="text-yellow-400 hover:text-yellow-300" title="Modifier">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-4 text-center text-gray-400">Aucun client trouvé</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
